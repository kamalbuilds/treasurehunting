...
import { TREASURY_CONTRACT_ADDRESS } from "../const/contractAddresses";
 ...
 * - Weaponss the connected wallet has
...
export default function OwnedGear({ weaponContract, miningContract }: Props) {
  ...
  const { data: ownedWeaponss, isLoading } = useOwnedNFTs(
    weaponContract,
    address
  );
  ...
  async function equip(id: string) {
    ...
    if (!hasApproval) {
      await weaponContract.setApprovalForAll(TREASURY_CONTRACT_ADDRESS, true);
    }
